<?php
// includes/backup_manager.php ù Respaldos e ùndice para el mùdulo histùrico
...
// =============================================================================
// Respaldos por fecha de producciùn (un archivo por dùa)
// =============================================================================

/** Agrupa lùneas del CSV origen por fecha de producciùn (columna Date). */
...
/** Elimina todos los respaldos asociados a una fecha de producciùn. */
...
/**
 * Detecta fechas en el CSV entrante y crea/reemplaza un respaldo diario por cada dùa.
 * Si el dùa ya existùa en backups, lo sustituye por el nuevo contenido.
 */
function syncProductionDateBackupsFromCsv(string $sourcePath): array
{
    ...
    if (!$split) {
        $result['error'] = 'No se detectaron fechas de producciùn en el CSV';
        return $result;
    }
    ...
    foreach ($split['by_date'] as $prodDate => $lines) {
        ...
        if ($hadBackup) {
            ...
            logMessage("Respaldo reemplazado para producciùn $prodDate (" . count($lines) . " filas)");
        } else {
            $result['created'][] = $prodDate;
            logMessage("Respaldo nuevo para producciùn $prodDate (" . count($lines) . " filas)");
        }
    }
...
// =============================================================================
// Crear respaldos (copia completa ù uso legacy)
// =============================================================================
...
function backupCSV(string $filepath, ?string $timestamp = null): bool
{
    ...
    if (!@copy($filepath, $dest)) {
        logMessage("backupCSV: fallù copy() ? $dest", 'error');
        return false;
    }
...
/** Cierra el dùa: crea BACKUP_YYYYMMDD_2359 si faltù (ayer o hoy en ventana nocturna). */
function finalizeDailyBackups(?string $sourceFile = null): array
{
    ...
    foreach (array_unique($targets) as $dateYmd) {
        ...
        if (backupCSV($source, $compact . '_2359')) {
            $done[] = $compact . '_2359';
            logMessage("Respaldo diario histùrico: $dateYmd");
        }
    }
 ...
 * - Diario 23:59 (o recuperaciùn al dùa siguiente)
 * - Incremental cada BACKUP_INTERVAL_SEC si cambiù el CSV en REPORTS
...
// =============================================================================
// Histùrico por fecha de producciùn
// =============================================================================
...
/** Lista para api.php?action=backups_by_date (por fecha de producciùn). */
...
// Compatibilidad con cùdigo existente
